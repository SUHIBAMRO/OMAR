"""FROZEN ARCHIVE -- B8-prototype/pilot (2026-09-22/23).

Per explicit instruction (2026-09-23): this is the exact code that
produced the real, GPU-confirmed pilot result (region-Cauchy field
error 7.53% at 791,864 elements, inside the advisor's 10^5-10^6-element/
5-10% target). It is kept UNMODIFIED and self-contained (imports its own
frozen data_generate_B8_prototype.py, not the revised data_generate_B8.py)
so that result stays independently reproducible while B8-final is built
with real published-source geometry/materials and a corrected,
material-boundary-respecting stress QoI region. Two real issues in this
prototype, NOT fixed here (fixed in the *-final files instead): (1) the
100x shim/rubber stiffness ratio was an arbitrary modeling shortcut, not
the real ~1e5 steel/rubber contrast, let alone real linear-elastic
steel; (2) the region-Cauchy QoI region was centered at the shim's own
mid-height and its cross-mesh comparison interpolated a field ACROSS
the rubber-steel material discontinuity, which is not a clean QoI
definition.

Original docstring (2026-09-22): Mesh-convergence study for B8 (3D
laminated annular elastomeric seismic bearing -- Option B). Per Omar's
explicit correction: a two-resolution smoke test is NOT a real result
-- this module is the real study: a genuine resolution ladder against a
fine reference, with the SAME symmetric, volume-weighted, quadrature-
based region-Cauchy-field methodology already validated for B3
(mesh_convergence_B3.py), adapted to this geometry's own (non-uniform,
banded) z-axis and its own reference point (the free edge of an
internal rubber-shim interface, not a groove).

Physics/BCs/material: see data_generate_B8_prototype.py's own docstring
for the full design and the real API-checked reasoning behind modeling
shims as per-element vectorized Neo-Hookean parameters rather than an
unsupported rigid multi-point constraint or torch-fem's shell-only
Laminate class.
...
from omar_pfem.data.data_generate_B8_prototype import (
