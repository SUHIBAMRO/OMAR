...
from plot_style import GEOMETRY_COLOR, add_bar_labels
...
add_bar_labels(ax, bars, fmt='{:.0f}s', fontsize=7)
...
bars = ax.bar(x, mem, color=colors)
ax.set_xticks(x)
ax.set_xticklabels(short, rotation=45, ha='right', fontsize=8)
ax.set_ylabel('Peak GPU memory (MB)')
ax.set_title('Training memory footprint')
ax.grid(True, axis='y', alpha=0.25)
add_bar_labels(ax, bars, fmt='{:.0f}', fontsize=7)
ax.set_ylim(top=ax.get_ylim()[1] * 1.15)

from matplotlib.patches import Patch
fig.legend(handles=[Patch(color=GEOMETRY_COLOR['B1'], label='B1'),
                     Patch(color=GEOMETRY_COLOR['B2'], label='B2')],
