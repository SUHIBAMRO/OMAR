"""Re-applies round 17's Executive-Summary/Conclusion cross-link fix on
top of round 18's base (which added the real FEM time/memory table to
11.5), since round 18 regenerated Section 11 from the pre-Section-11
base and does not itself include round 17's in-place edits. Same fix,
same two locations, just re-run against the newer source file -- see
add_round17_executive_summary_and_conclusion_crosslinks.py's own
docstring for the original rationale, not repeated here.
...
SRC = os.path.join(DELIV, 'PFEM_Transolver_Report_2026-09-24d.docx')
DST = os.path.join(DELIV, 'PFEM_Transolver_Report_2026-09-24e.docx')
