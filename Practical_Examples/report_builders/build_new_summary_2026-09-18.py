...
elements = slice_by_anchor_text(
    ...
    "10.48% vs. region average 10.69%) -- stress is a genuinely harder, noisier target "
...
elements = slice_by_anchor_text(
    ...
    "all seven completed training runs: five multi-resolution retrains, one original B2 "
    "x Arruda-Boyce run (it has no multi-resolution retrain of its own -- see the "
    "table's own caption), and the direct-N1401 ablation just discussed -- read directly "
    "from each run's own already-saved metrics_history.json, nothing retrained for this "
    "table. Peak GPU "
