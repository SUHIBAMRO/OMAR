"""Builds the NEW Work Summary under Omar's own changed convention
(2026-09-18): from this point on, the Summary is NON-CUMULATIVE -- it
covers only the round just completed, not the whole project's history.
The Report stays the single cumulative record; the Summary is now a
disposable "what did we just finish" companion, rebuilt fresh each round
rather than carried forward and appended to.

This particular Summary covers round-12 (points 1, 2, 3), copied
VERBATIM out of the current Report (PFEM_Transolver_Report_2026-09-18.docx)
-- not re-derived or re-typed, so there is zero risk of the Summary
drifting from the already-verified Report content. The old, cumulative
PFEM_Work_Summary_2026-09-18.docx (round-10/11/12 all together) is kept
on disk as the last cumulative snapshot but is no longer the one to send
going forward.
...
OUT = os.path.join(DELIV, 'PFEM_Work_Summary_2026-09-18b.docx')
...
append_heading(dst, "PFEM / Transolver Work Summary -- Round 12")
p = dst.add_paragraph()
p.add_run(
    "Starting with this round, the Summary covers only the round just completed, not "
    "the project's full history -- the full Report (PFEM_Transolver_Report_2026-09-18.docx) "
    "remains the single cumulative record of everything done. This document contains "
    "the three points from Timon's round-12 email (2026-09-18), in his own point order, "
    "copied verbatim (same tables, same figures, same text) out of that Report -- nothing "
    "summarized or re-derived."
).italic = True
...
elements = slice_by_anchor_text(
    ...
    "asked for one final accuracy-versus-resolution table, FEM vs. operator vs. the same "
    "fine reference, in displacement L2, H1/energy norm, reaction force, and stress -- "
    "with Cauchy stress (not PK1) as the main engineering quantity, explicitly rejecting "
    "the bare pointwise maximum as the primary QoI (singular/mesh-dependent at corners "
    "even for FEM) in favour of a FIXED physical region around the stress concentration, "
    "reporting a robust local statistic (a weighted average or 99th percentile) with the "
    "true max still available separately, the region fixed in physical space across "
    "every resolution. All new code (fixed-region selection, the Cauchy push-forward "
    "from the already-validated PK1 stress, and the weighted-percentile/top-fraction "
    "statistics) was verified on CPU before any GPU time was spent (5 checks, all "
    "passed). Scope, stated plainly: resolutions here are the LOW_N range [3..49] "
    "against a fine reference at N=201, the same convention already used for this "
    "project's own coarsest-suitable-mesh crossover analysis above -- this does NOT "
    "extend to N=1401, which would need a fresh, expensive fine_N=2236 reference for the "
    "five cases that have never had one, a separate, bigger ask that should be scoped on "
    "its own rather than silently bundled in here.",
