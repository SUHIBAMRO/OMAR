...
entries (B1xNeo-Hookean, B1xMooney-Rivlin, B1xArruda-Boyce) and this
session's own real GPU log (B2xMooney-Rivlin) -- see
add_multires_retrain_extension_to_report.py for the exact same numbers
used in the accompanying tables, so the figure and the tables cannot
silently drift apart.

B2xNeo-Hookean is deliberately NOT plotted here: its own multi-res
retrain was still mid-training as of this figure's build date, so it
has no final N=1401 number yet.
"""
import os
...
CASES = ['B1 x Neo-Hookean', 'B1 x Mooney-Rivlin', 'B1 x Arruda-Boyce', 'B2 x Mooney-Rivlin']
OLD = [44.65, 39.20, 45.62, 49.47]
NEW = [5.85, 15.04, 25.05, 13.10]
