...
ax.bar(x - width / 2, pi_err, width, label='Physics-informed', color='#1f77b4')
ax.bar(x + width / 2, dd_err, width, label='Data-driven', color='#ff7f0e')
ax.set_xticks(x)
...
ax.bar(x - width / 2, pi_total, width, label='Physics-informed total', color='#1f77b4')
ax.bar(x + width / 2, dd_total, width, label='Data-driven total\n(incl. label generation)', color='#ff7f0e')
ax.set_yscale('log')
ax.set_xticks(x)
ax.set_xticklabels(OPT_SHORT, fontsize=9)
