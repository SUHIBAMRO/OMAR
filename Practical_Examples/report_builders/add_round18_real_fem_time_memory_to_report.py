"""Round 18 -- fills in the one number that round 16 explicitly left
pending: real, per-resolution FEM wall-clock time and GPU peak memory for
the B3 (Candidate A) ladder, from Omar's real GPU re-run of the
instrumented cell (commit d1ca402). The QoI numbers (region-Cauchy field
error, displacement L2) in this real run are IDENTICAL to the ones
already reported in 11.2 -- confirmed by direct comparison against
PROJECT_STATUS.md before writing anything below, not assumed -- so 11.2's
own table is untouched; only 11.5 changes, from "not yet available" to
the real table.

Same regeneration strategy as rounds 15/16: rebuilds Section 11 from the
pre-Section-11 base, with 11.1-11.4 reproduced unchanged (still accurate)
and 11.5 revised to replace the pending-memory paragraph with the real
time/memory table and an updated closing paragraph (dataset generation
still has not started, so that sentence is kept, just no longer
conditioned on the time/memory table being "complete").
"""
import os
...
REPORT_DST = os.path.join(DELIV, 'PFEM_Transolver_Report_2026-09-24d.docx')
...
para(
    ...
    "meshes, not a new GPU run. Second, real per-resolution GPU peak "
    "memory (peak allocated and peak reserved, via torch.cuda's own "
    "peak-memory-stats API around each solve) was added to the mesh-"
    "convergence code and the ladder re-run on an A100 to obtain it -- "
    "the region-Cauchy field error and displacement L2 values in this "
    "re-run are identical to 11.2's own table, confirming reproducibility "
    "before trusting the new memory numbers alongside them."
)

table(
    ["Elements", "Time (s)", "GPU peak allocated (MB)",
     "GPU peak reserved (MB)"],
    [
        ["336", "5.21", "32.8", "52.4"],
        ["1,320", "4.01", "79.1", "109.1"],
        ["3,360", "4.45", "174.7", "213.9"],
        ["6,840", "4.86", "338.7", "419.4"],
        ["18,200", "5.86", "866.4", "1,050.7"],
        ["79,464", "13.52", "3,689.4", "4,538.2"],
        ["201,780", "30.46", "9,299.0", "11,395.9"],
        ["480,320", "76.06", "22,073.0", "27,086.8"],
        ["557,760", "89.40", "25,631.4", "31,436.3"],
        ["643,104", "107.71", "29,550.1", "36,251.4"],
        ["736,736", "125.89", "33,841.8", "41,517.3"],
        ["839,040", "149.56", "38,529.2", "47,271.9"],
        ["950,400 (reference)", "176.90", "43,637.5", "53,540.3"],
    ],
)

para(
    "Both time and peak memory grow substantially faster than linearly "
    "with element count (roughly 2.4x the elements from 480,320 to "
    "950,400 elements costs about 2.3x the time and 2.0x the peak "
    "memory), consistent with the increased fill-in of the assembled "
    "tangent stiffness matrix at finer 3D meshes. The full 13-point "
    "ladder completed in 41 minutes 17 seconds total on a single A100 -- "
    "consistent with the original run's 40 minutes 41 seconds, the small "
    "difference being ordinary run-to-run variation, not a regression."
)
...
para(
    "No dataset generation or neural-operator training has started. All "
    "three of the advisor's next-step items above are now addressed "
    "(reference chosen, time/memory reported, VINO parameter scope "
    "decided) except the exact sampling ranges for E, nu, and phi, which "
    "remain a deliberately separate, not-yet-decided step. Once those "
