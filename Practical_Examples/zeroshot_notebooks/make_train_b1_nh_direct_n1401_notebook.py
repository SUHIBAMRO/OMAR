    ...
    return {
        ...
        "cells": [
            md(
                ...
                "",
                "**تصحيح رابع، نفس اليوم**: `batch_size=1` يعني كل epoch "
                "فيها 100 خطوة (بدل ~12 عند batch=8) -- وبـ"
                "`validate_every=25` (قيمة الـmulti-res الأصلية)، أول طباعة "
                "كانت رح تتأخر 2500 خطوة، يعني ساعات من السكوت الكامل قبل "
                "أي تأكيد إنه التدريب شغال صح. نزّلناها لـ`validate_every=2` "
                "(200 خطوة بس لأول طباعة) -- تسريع بالتغذية الراجعة بس، ما "
                "بغيّر شي بالنتيجة النهائية.",
                "",
                "**تصحيح خامس، نفس اليوم**: التصحيح الرابع نجح فعلاً بتحريك "
                "التدريب (batch=1 + grad_checkpoint + allocator كلهم شغلوا) "
                "-- بس طلع OOM **جوا التحقق (validation)**، مش التدريب. "
                "السبب: دالة `evaluate_resolution` كانت دايماً بتحط **كل** "
                "عينات التحقق (20 هون) بدفعة وحدة، بغض النظر عن "
                "`--batch_size` -- عند N=1401 هاد يعني (20، ~1.97 مليون "
                "عقدة) بضربة وحدة، أكبر بكثير من إعداد التدريب نفسه. "
                "أصلحناها بشكل عام (`--eval_chunk_size 1`) -- بتقسّم دفعة "
                "التحقق لقطع صغيرة وتجمع النتائج، **بدون أي تغيير بالنتيجة "
                "النهائية** (اتفحص على CPU: نفس الرقم بالضبط بكل أحجام "
                "القطع).",
                "",
