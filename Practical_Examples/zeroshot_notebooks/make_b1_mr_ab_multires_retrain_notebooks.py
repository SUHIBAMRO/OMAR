    ...
    return {
        ...
        "cells": [
            ...
            code(
                ...
                "# Real bug found 2026-09-14 on a sibling notebook: JAX (used for this",
                "# material's PK1/tangent autodiff) defaults to preallocating ~90% of the",
                "# WHOLE GPU the moment it first touches one, invisible to torch.cuda's own",
                "# memory stats, starving the actual training/data-generation for the rest",
                "# of the session. material_models_jax.py already sets this via setdefault,",
                "# but that has no effect if jax is already initialized elsewhere first --",
                "# forcing it here, before anything else runs (including the !python -m ...",
                "# shell cells below, which inherit this same process's environment).",
            ...
            code(
                ...
                "",
                "# Real bug found 2026-09-15: خلية 3 (التدريب) بتخلّي الـmodel/optimizer/",
                "# cached activations متل ما هي بالـGPU لأنها بنفس الـkernel، وهاد الخلية",
                "# بعدها بتحتاج تعمل sparse direct solve (cuDSS) عند N=1001/1401 يلي محتاج",
                "# ذاكرة GPU كبيرة -- سبب حقيقي شفناه: cuDSSError: ALLOC_FAILED عند N=1001.",
                "# تنظيف صريح هون قبل أي شي، قبل حتى استدعاء run_one أول مرة.",
                "gc.collect()",
                "torch.cuda.empty_cache()",
                "torch.cuda.reset_peak_memory_stats()",
                "print(f'GPU مُتاحة بعد التنظيف: {torch.cuda.mem_get_info()[0] / 1e9:.1f} GB حرة من "
                "{torch.cuda.mem_get_info()[1] / 1e9:.1f} GB')",
                "",
                ...
                "    finally:",
                "        # نفس السبب: لازم نحرر الـmodel القديم قبل ما نحمّل الجديد، وإلا",
                "        # الاثنين بيضلو محجوزين بالـGPU وقت أكبر sparse solve (N=1001+).",
                "        del model",
                "        gc.collect()",
                "        torch.cuda.empty_cache()",
                "",
