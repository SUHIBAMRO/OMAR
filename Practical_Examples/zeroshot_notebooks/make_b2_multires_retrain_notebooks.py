    ...
    return {
        ...
        "cells": [
            ...
            md("## خلية 2 — توليد بيانات FEM (بالحل السريع)",
               ...
               "",
               "**تصحيح حرج، 2026-09-16 (لقيناه من لوجات حقيقية على Colab)**: "
               "`--fast_solver 1 --nsteps 3` بهاي الخلية **كانوا بدون أي تأثير "
               "فعلي على B2** من أول يوم -- `cmd_train` كانت بتفعّل الحل السريع "
               "(`solve_b1_fast_gpu`) بس لما `geometry == B1`؛ لـB2 كانت دايماً "
               "بتستخدم `build_sample_b2` العادية يلي بتنادي الحل البطيء "
               "(`solve_hyperelastic_TL_ring`، حلقة Python لكل عنصر + "
               "scipy sparse solve على الـCPU) وفيها `nsteps=10` **مكتوبة "
               "بالكود مباشرة**، مش مرتبطة بأي CLI flag. اكتُشفت لما لوجات "
               "حقيقية ضلت تطبع `[step X/10]` مهما فُتح تاب جديد أو اتصلنا "
               "بـruntime جديد. **الإصلاح**: بنينا `build_sample_b2_fast` "
               "(نسخة طبق الأصل عن `build_sample_b1_fast`) تستخدم "
               "`solve_b2_fast_gpu` (موجودة أصلاً من 2026-09-14، بس ما كانت "
               "متوصولة بخط أنابيب التوليد)، ووصلناها بـ`cmd_train`. "
               "**اتحقق منها فعلياً على CPU قبل الثقة فيها** (نفس انضباط "
               "المشروع): عند N=11 (nsteps=10 الاثنين) الفرق مع الحل البطيء "
               "= **صفر تماماً** (bit-identical)؛ عند N=21 (nsteps=3 السريع "
               "مقابل nsteps=10 البطيء، Neo-Hookean وMooney-Rivlin) الفرق "
               "النسبي ~1e-8/1e-9 (متوقع، من اختلاف خطوات التحميل نفسها، مش "
               "خطأ). **السرعة حتى على CPU لحاله**: N=21 Neo-Hookean من "
               "15.44 ثانية (بطيء) لـ0.66 ثانية (سريع) -- ~23x أسرع، وعلى "
               "GPU حقيقي متوقع أسرع بكثير كمان (نفس نمط B1: من ساعات "
               "لثواني). **هاد الإصلاح بالكود بس -- سطر الأمر بهاي الخلية "
               "ما تغيّر إشي**، كان أصلاً `--fast_solver 1 --nsteps 3`؛ "
               "بس هلق فعلاً بيشتغل متل ما كان المفروض من البداية.",
               "",
               "**بتحفظ على Drive كل 25 عينة** — إذا وقف النوتبوك، رجّع شغّل",
               "نفس الخلية وبتكمّل من وين وقفت. عينات اتولدت قبل هالإصلاح "
               "(بـnsteps=10 عبر الحل البطيء) بتضل صحيحة ومتوافقة -- بس حل "
               "مختلف، مش نتيجة نهائية مختلفة."),
            code(
