    ...
    return {
        ...
        "cells": [
            md(
                ...
                "",
                "**تحديث رابع، نفس اليوم -- باگ حقيقي بالـcutback نفسه "
                "(انلقى من تشغيلة عمر الحية) وانصلح**: استثناء (exception) "
                "من الحل الخطي (AssertionError لما القطر يصير سالب) كان "
                "بيهرب من كل حلقة الـcutback بدل ما يفعّلها -- صلّحناها "
                "(commit f052493). **بس** بعدها بتشغيلة تانية، (45,23) و"
                "(53,27) فشلوا **بنفس الرسالة تمامًا** وبدون أي رسالة "
                "\"[cutback]\" ظاهرة بالسجل. راجعنا الكود بعناية: الإصلاح "
                "صحيح بنيويًا. **السبب الحقيقي الأرجح**: قبل ما يوصل "
                "للفشل، نيوتن كان يحاول حل خطي CG كامل على حالة متباعدة "
                "أصلًا -- ووصل هالحل لـ27,363 دورة CG بمحاولة وحدة (تأكد "
                "حي عند 105,456 عنصر) -- كمية سجلّ ضخمة جدًا، وColab "
                "بيقص السجلات الكبيرة تلقائيًا (بيحذف النص من النص "
                "غالبًا)، فرسالة الـ[cutback] الأرجح انحذفت بالقص مش إنها "
                "ما طبعت أصلًا. **إصلاح حقيقي إضافي**: أضفنا كشف تباعد "
                "مبكر -- إذا البقايا (residual) قفزت أكتر من 10 أضعاف "
                "بخطوة نيوتن وحدة (شفناها فعليًا تقفز 19-540 ضعف بالسجلات "
                "الحقيقية)، نعمل cutback فورًا **بدون** إضاعة حل خطي كامل "
                "على حالة ميؤوس منها -- بيوفر وقت GPU حقيقي وبيخلي السجل "
                "أقصر وأوضح. تحقّقنا محليًا: نفس الجواب بالضبط (فرق "
                "1.6e-6) وأسرع.",
            ),
