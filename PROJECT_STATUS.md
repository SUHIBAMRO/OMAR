...
Last updated: 2026-09-07 (latest: item #4's multigrid preconditioner is
now CONFIRMED working on real GPU data at N=401 — cg_failures 20→0,
wall_clock ~43.6 min (down from the pre-fix rewrite's ~87 min projection,
and only ~1.6x the old non-converging Jacobi run's ~27 min). Got there by
diagnosing and fixing a serial-loop/insufficient-coarsening bottleneck in
`multigrid_precond.py` (found right after the first real N=401 run showed
correct-but-82x-slower CG), re-verifying correctness on CPU before
