...
| 7 | New study: train a DD-NO on FEM labels from a coarse mesh vs. a finer mesh; measure how its accuracy/generalization across resolutions changes | R8.3 | 🟡 **Notebook built and locally verified, budget-tag bug found and fixed 2026-09-06, not yet run for real at the correct budget.** `zeroshot_notebooks/cell_dd_no_coarse_vs_fine.py` / `Round6_DD_NO_Coarse_vs_Fine.ipynb`. B1×Neo-Hookean, N=13 (coarse) vs. N=33 (fine), zero-shot evaluated at Table 12's same seven resolutions. **Budget fixed** to Table 21's exact 800/200/75,000, identical for both COARSE and FINE (an earlier draft would have confounded label-resolution with training budget by giving fine the full budget and coarse a smaller pilot one — Omar caught this). **Second bug found the same day, when Omar actually ran the fixed notebook on Colab**: every stage printed `[skip] ... already done` and the run finished in seconds, because the old pilot-scale run (200/50/20,000, from before the budget fix) had already written to `{tag}_N{N}_raw`/`_q4`/`_train`/`_zeroshot.json` on Drive under the SAME paths the new 800/200/75,000 run uses — the resumability check only asks "does this path exist," not "was it built at the current budget," so it silently returned stale pilot-scale results. Fixed by keying every stage past calibration to a `BUDGET_TAG` (`800tr_200te_75000steps`) baked into the directory name, so a budget change always starts a fresh directory instead of adopting whatever already sits at the old path. Rebuilt and re-verified (`make_round6_notebooks.py`, `check_notebooks.py`, 39/39 OK). **The `coarse_N13_zeroshot.json`/`fine_N33_zeroshot.json` Omar already has on Drive are the stale pilot-budget results — do not use them for the report; the notebook needs to be run again from this fixed version to get real 800/200/75,000-budget numbers** | Needs Omar to re-run it for real on Colab (GPU preferred) — the previous run's output is stale, see above |
