...
14 is now CONFIRMED** — Omar re-uploaded the PDF directly and it is the
same paper Timon sent, but it is still NOT the GOEE paper by name or
method (no goal-oriented/adjoint content at all — see item 14's row for
what it actually is and how to cite it); **items 6 and 7 are now DONE**
with real GPU numbers (OOD progressive-shift's last 5 cases, and DD-NO
coarse-vs-fine zero-shot) folded in below. **Blocker found 2026-09-07**:
Omar asked to write items 6/7's numbers and item 14's citation into the
actual Word-format Report and Summary documents — searched the git repo
(`find -iname "*.docx"`, empty) and Google Drive (6 different queries by
title/mimeType/fullText) and could NOT locate `PFEM_Transolver_Report_v56.docx`
or its Summary counterpart anywhere accessible from this session. Asked
Omar where these files actually live / to upload current versions —
nothing can be written into the real report/summary until then.
