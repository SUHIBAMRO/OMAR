...
file Omar sends out. Items 6/7/14 were added directly to whatever this
real file actually contains (verified anchors, not the assumed v56
baseline) via two new scripts, `report_builders/add_items_6_7_14_report.py`
and `add_items_6_7_14_summary.py` — see their docstrings for the full
version-gap note. **Not yet resolved**: whether Omar wants the #1/#2/#5/#8
backfill done too (a separate, larger job, since the existing v53-56
scripts' anchors don't cleanly apply to this older file and would need
re-verification one at a time). The updated files were sent back to Omar
directly this session; the version-gap question is still open.
