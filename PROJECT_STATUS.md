...
Last updated: 2026-09-08 (**item #13's real GPU sweep FINISHED**, after
five real issues found and fixed one after another on the way there —
see item #13's row for the full blow-by-blow (pyvista/IPython, two
separate torch-fem internal device-default bugs, a stale in-kernel
module cache, and finally a stale browser tab never picking up any of
the fixes until given a fresh `colab.research.google.com/github/...`
link). **The headline result, N=401/701/1001**: torch-fem is
dramatically faster in wall-clock than this project's own matrix-free
mgv solver (~418x/655x/1197x, an advantage that GROWS with N) — real,
but reported with two honest caveats front and center: the two solvers
are not run at matched precision/tolerance (torch-fem forced to float32
with loose tolerances vs. our float64/tight), and "ours" own peak GPU
memory was never actually captured this run (it resumed from a
checkpoint by design, so there is nothing real yet to compare against
torch-fem's own measured, committed memory numbers). Two open decisions
for Omar, not further debugging: extend to N=1401 (now proven to be
near-free), and whether a real (costly) "ours" memory measurement is
worth doing given the wall-clock result already answers Timon's
question honestly on its own.)
