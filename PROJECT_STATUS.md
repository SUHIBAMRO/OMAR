...
| 7 | New study: train a DD-NO on FEM labels from a coarse mesh vs. a finer mesh; measure how its accuracy/generalization across resolutions changes | R8.3 | 🟢 **LIVE since the morning of 2026-09-06 on Omar's own Colab tab — confirmed by checking Drive directly, not assumed.** `zeroshot_notebooks/cell_dd_no_coarse_vs_fine.py` / `Round6_DD_NO_Coarse_vs_Fine.ipynb`, B1×Neo-Hookean, N=13 (coarse) vs. N=33 (fine), zero-shot at Table 12's seven resolutions, budget fixed to Table 21's exact 800/200/75,000 (identical for both), and the earlier stale-checkpoint bug fixed via `BUDGET_TAG`-keyed directory names (see prior entries in this file's history for both bugs' full stories). **COARSE (N=13) is fully done, with real numbers already on Drive**: `coarse_N13_800tr_200te_75000steps_zeroshot.json`, mean rel. L2 at the 7 test resolutions (13/17/25/29/37/41/49) = 0.105/0.117/0.154/0.174/0.211/0.228/0.259 — a clean, expected pattern (error rises moving away from the N=13 training resolution). **FINE (N=33) has just started** (its `_raw` data-generation folder was created ~21:39, no `_q4`/`_train`/`_zeroshot` yet) — expect roughly the remaining ~5 h of the ~6-6.5 h total estimate (fine's own FEM data generation, ~4h44min, is the dominant remaining cost). ⚠️ **Do not open a second tab/run for this notebook while the existing one is live** — both would write to the same Drive paths and risk a conflict, exactly the kind of duplicate-write risk this file's own history already flagged once before (see the "container restart" incident, 2026-09-01) | Nothing to do but let the existing tab keep running; check back once `fine_N33_..._zeroshot.json` appears on Drive, then compare both resolutions' zero-shot columns against each other and against Table 12's physics-informed numbers |
