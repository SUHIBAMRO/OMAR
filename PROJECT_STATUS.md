...
| 7 | New study: train a DD-NO on FEM labels from a coarse mesh vs. a finer mesh; measure how its accuracy/generalization across resolutions changes | R8.3 | 🟡 **Notebook built AND locally verified end-to-end**, not yet run for real: `zeroshot_notebooks/cell_dd_no_coarse_vs_fine.py` / `Round6_DD_NO_Coarse_vs_Fine.ipynb`. B1×Neo-Hookean, N=13 (coarse) vs. N=33 (fine), zero-shot evaluated at Table 12's same seven resolutions. All four stages (generate → convert → train → eval) were run for real at tiny scale (N=13/17, 2–4 samples, 4 optimizer steps) and completed with no errors — confirmed `resolution_invariance_zeroshot.py eval` correctly loads and evaluates a DD-NO checkpoint, not just a physics-informed one. Runs a 20-sample calibration at each resolution FIRST and prints the real measured per-sample cost before the full run. **Budget fixed 2026-09-06**: Omar caught that an earlier draft would have run fine at Table 21's full 800/200/75k while leaving coarse at a smaller pilot budget (200/50/20k) — a training-budget confound on top of the intended label-resolution comparison. Both COARSE and FINE now use the identical 800/200/75,000 budget, matching Table 21 exactly; only the label-generation mesh (N=13 vs. N=33) differs between the two runs. Notebook rebuilt and re-verified (`make_round6_notebooks.py`, `check_notebooks.py`, 39/39 OK) | Needs Omar to run it for real on Colab (GPU preferred) |
