...
Last updated: 2026-09-07 (latest: item #4's multigrid preconditioner
converges on the real N=401 GPU problem but was ~82x more expensive per
CG iteration than the old non-converging run; diagnosed and fixed a
serial-loop/insufficient-coarsening bottleneck in `multigrid_precond.py`,
re-verified correctness on CPU, awaiting Omar's GPU re-run to confirm the
real speedup — see item #4's row and the summary above. Earlier the same
day: items 5 and 8 done directly
