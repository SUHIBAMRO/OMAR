...
Last updated: 2026-09-08 (**item #13 in progress**: real Colab run hit
THREE issues one after another, all diagnosed and fixed — (1)
`pyvista`/IPython import failure on Colab, fixed by stubbing the whole
`pyvista` module with a `MagicMock`, confirmed working by Omar's own
run; (2) a genuine device-mismatch bug INSIDE torch-fem's own
constructor (`torch.arange` with no explicit device), fixed by wrapping
the model construction in `with torch.device(device):`; (3) after that
fix was pushed, Omar got the EXACT SAME error again — turned out to be a
STALE in-kernel Python module cache in the long-lived Colab runtime
(`git reset --hard` updates files on disk, not `sys.modules` in an
already-running kernel), not a new code bug — fixed by purging
`omar_pfem.*` from `sys.modules` at the top of the sweep cell before
importing, so every run gets the real current code regardless of what
an earlier cell already imported. Rebuilt/verified 45/45 notebooks. **After that fix, Omar's re-run got
PAST the stale-cache issue (fresh clone this time, so it couldn't
recur) but hit a FOURTH, related bug**: the same
device-defaults-to-CPU pattern as (2), this time inside torch-fem's own
`near_null_space()`/`skew()` (called from `.solve()` itself, not the
constructor) -- `torch.zeros`/`torch.arange`/`torch.eye` built with no
device there too, then `torch.cat` against `self.nodes` (which IS on
the right device) fails the same way. Fixed by widening the existing
`with torch.device(device):` wrap in `solve_theirs` to cover the whole
`model.solve(...)` call, not just the constructor -- this also covers
any other device-less internal call anywhere in torch-fem's `solve()`
call tree, not just this one. Re-verified the CPU correctness check
still passes (PASS, 4.608e-06 rel. diff, unchanged). Omar needs to
re-run `Round6_TorchFEM_Comparison.ipynb` with this latest fix. See
item #13's row for the full story.)
